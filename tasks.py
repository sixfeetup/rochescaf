...
def remove_celery_files():
    file_names = [
        os.path.join("backend", "rochescaf", "celery.py"),
        os.path.join("backend", "rochescaf", "users", "tasks.py"),
        os.path.join(
            "backend",
            "rochescaf",
            "users",
...
def configure_git_remote():
    repo_url = "git@github.com:sixfeetup/rochescaf.git"
    if repo_url:
...
def party_popper():
    ...
    print("\r🎊 Congrats! Your rochescaf project is ready! 🎉")
    print()
...
def run_setup():
    subprocess.run(shlex.split("kind create cluster --name rochescaf"))
