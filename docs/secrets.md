...
Configure kubernetes to your current project config and context, making sure you are in the correct prod/sandbox environment
    ...
    $ export KUBECONFIG=~/.kube/config:~/.kube/rochescaf.ec2.config
    $ kubectl config use-context rochescaf-environment
...
The decrypted values can be retrieved running:
    ...
    $ kubectl get secret secrets-config -n rochescaf -o yaml > unsealed_secrets.yaml
